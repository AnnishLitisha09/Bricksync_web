...
const { emitTelemetry } = require("../utils/socket");
...
exports.getVehicleLiveData = async (req, res) => {
    try {
                ...
                try {
                    ...
                    if (result.code === 1 && Array.isArray(result.data)) {
                        ...
                        if (vehicle) {
                            ...
                            res.json(mergedVehicle);
                        } else {
                            res.status(404).json({ message: "Vehicle not found in live data" });
                        }
                    } else {
                        res.status(500).json({ message: "External API error", detail: result.msg });
                    }
        ...
        externalReq.on("error", (err) => res.status(500).json({ error: err.message }));
        externalReq.write(postData);
        externalReq.end();
    } catch (err) {
        res.status(500).json({ error: err.message });
