...
const Banks: React.FC = () => {
    const { banks, loading, fetchBanks, updateBank, deleteBank } = useBankStore();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setFormData({ id: undefined, name: "", accountNumber: "", holderName: "", amount: "", bankTransfer: true, phonepe: false, gpay: false });
        } catch (error) {
    ...
    const BankCard = ({ bank }: { bank: any }) => {
        ...
        const handleEditClick = () => {
            setFormData({
                ...
                amount: String(deobfuscate(bank.amount)),
                bankTransfer: bank.bankTransfer,
                phonepe: bank.phonepe,
                gpay: bank.gpay
            });
            setIsModalOpen(true);
        };
        ...
        return (
            ...
            >
                <div className="absolute top-0 right-0 p-6 flex gap-2 z-10 transition-all group-hover:opacity-0">
                ...
                <div className="absolute top-0 right-0 p-6 flex gap-2 z-20 opacity-0 group-hover:opacity-100 transition-all">
                    <button onClick={handleEditClick} className="bg-white/20 hover:bg-white/30 backdrop-blur-md p-2 rounded-xl border border-white/20 transition-all text-white"><Pencil size={16} /></button>
                    <button onClick={() => setDeleteId(bank.id)} className="bg-red-500/20 hover:bg-red-500/40 backdrop-blur-md p-2 rounded-xl border border-red-500/20 transition-all text-red-200"><X size={16} /></button>
                </div>
            ...
            <AnimatePresence>
                ...
                {deleteId && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setDeleteId(null)} className="absolute inset-0 bg-slate-900/60 backdrop-blur-xl" />
                        <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white w-full max-w-sm rounded-[3rem] shadow-2xl overflow-hidden p-10 text-center space-y-6">
                            <div className="mx-auto w-20 h-20 bg-red-50 rounded-3xl flex items-center justify-center text-red-500">
                                <X size={40} />
                            </div>
                            <div className="space-y-2">
                                <h3 className="text-2xl font-black uppercase tracking-tighter">De-link Account?</h3>
                                <p className="text-slate-400 font-bold text-xs uppercase tracking-widest px-4">This action will permanently remove this vault entry from the network.</p>
                            </div>
                            <div className="flex gap-3">
                                <button onClick={() => setDeleteId(null)} className="flex-1 py-4 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all">Cancel</button>
                                <button onClick={handleDelete} disabled={isDeleting} className="flex-1 py-4 bg-red-600 hover:bg-red-700 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all shadow-lg shadow-red-200 flex items-center justify-center gap-2">
                                    {isDeleting ? <Loader2 size={14} className="animate-spin" /> : "De-link"}
                                </button>
                            </div>
                        </motion.div>
                    </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden p-8 md:p-10 space-y-6 border border-slate-100">
                            <div className="flex justify-between items-center">
                                <div><h2 className="text-3xl font-black uppercase tracking-tighter leading-none">{formData.id ? "Edit Protocol" : "Vault"}</h2><p className="text-orange-600 font-black text-[10px] uppercase tracking-widest mt-1">{formData.id ? "Update Configuration" : "Initialize Protocol"}</p></div>
                                <button onClick={() => {
                                    setIsModalOpen(false);
                                    setFormData({ id: undefined, name: "", accountNumber: "", holderName: "", amount: "", bankTransfer: true, phonepe: false, gpay: false });
                                }} className="p-3 hover:bg-slate-100 rounded-full transition-colors text-slate-400"><X size={24} /></button>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <button disabled={isSubmitting} type="submit" className="w-full py-5 bg-orange-600 hover:bg-orange-700 text-white rounded-[2rem] font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 transition-all shadow-xl shadow-orange-200">
                                    {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : <> <CheckCircle2 size={18} /> {formData.id ? "Update Configuration" : "Link Secure Account"} </>}
                                </button>
                            </form>
                        </motion.div>
                    </div>
