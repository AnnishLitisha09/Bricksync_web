...
export default function FuelPage() {
  ...
  // Fetch data on mount and whenever the store's page would change
  useEffect(() => {
    getFuels(currentPage);
    fetchVehicles();
  ...
  return (
    ...
    >
      ...
      {/* QUICK STATS (Calculated from current page data) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Page Volume" value={`${fuels.reduce((acc, curr) => acc + (curr.volume || 0), 0).toLocaleString()} L`} icon={<Droplets className="text-blue-500" />} />
        <StatCard title="Page Expenditure" value={`₹${fuels.reduce((acc, curr) => acc + (curr.amount || 0), 0).toLocaleString()}`} icon={<CreditCard className="text-emerald-500" />} />
      </div>
