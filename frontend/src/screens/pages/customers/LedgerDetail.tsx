...
import { 
  ...
  Download, Receipt, Loader2, Truck, Plus, Wallet, PhoneCall, X 
} from "lucide-react";
...
const CustomerDetails: React.FC = () => {
  ...
  const [callDetails, setCallDetails] = useState({ date: new Date().toISOString().split('T')[0], remark: "" });
  ...
  const handleCallSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Call Logged:", callDetails);
    // Add your API call here to save the remark
    setIsModalOpen(false);
    setCallDetails({ date: new Date().toISOString().split('T')[0], remark: "" });
  };
  ...
  if (!customer) return <div className="p-12">Customer not found.</div>;
    ...
    <div className="p-4 sm:p-8 lg:p-12 min-h-screen bg-[#FBFDFF] space-y-8 animate-in fade-in duration-500 relative">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  ...
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Item Particulars & Transport</th>
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Debit (In)</th>
                  <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Credit (Out)</th>
                </tr>
              </thead>
                    ...
                    <td className="px-8 py-6">
                      <div className="space-y-6">
                        {tx.items.map((item: any, idx: number) => (
                          <div key={idx} className="flex flex-col border-l-2 border-slate-100 pl-4 gap-1">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl relative z-10 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-black text-slate-900">Log Call Remark</h3>
            ...
            <form onSubmit={handleCallSubmit} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Call Date</label>
                <input 
                  type="date" 
                  required
                  className="w-full px-4 py-4 rounded-2xl border border-slate-200 bg-slate-50 font-bold text-slate-700 focus:ring-4 focus:ring-indigo-500/5 outline-none"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Remark / Discussion</label>
                <textarea 
                  placeholder="What did the client say about payment?"
                  required
                  rows={4}
                  className="w-full px-4 py-4 rounded-2xl border border-slate-200 bg-slate-50 font-bold text-slate-700 focus:ring-4 focus:ring-indigo-500/5 outline-none resize-none"
              ...
              <button 
                type="submit"
                className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black hover:bg-indigo-600 transition-all shadow-xl"
              >
                Save Call Entry
              </button>
            </form>
