...
const AddInvoice: React.FC = () => {
    ...
    // --- CALCULATIONS ---
    const subTotalValue = items.reduce((sum, item) => sum + (Number(item.quantity || 0) * Number(item.rate || 0)), 0) || 0;
    const totalTax = (subTotalValue * (Number(invoiceData.sgstRate) || 0)) / 100;
    const finalAmount = subTotalValue + (totalTax * 2);
    const grandTotal = Math.round(isFinite(finalAmount) ? finalAmount : 0);
    const roundOffValue = (isFinite(finalAmount) ? (grandTotal - finalAmount).toFixed(2) : "0.00");

    const numberToWords = (num: number): string => {
        if (!isFinite(num)) return 'INVALID AMOUNT';
        ...
        const convert = (n: number): string => {
            if (n < 20) return ones[Math.floor(n)];
            if (n < 100) return tens[Math.floor(n / 10)] + (Math.floor(n % 10) !== 0 ? ' ' + ones[Math.floor(n % 10)] : '');
            if (n < 1000) return ones[Math.floor(n / 100)] + ' HUNDRED' + (Math.floor(n % 100) !== 0 ? ' AND ' + convert(Math.floor(n % 100)) : '');
            if (n < 100000) return convert(Math.floor(n / 1000)) + ' THOUSAND' + (Math.floor(n % 1000) !== 0 ? ' ' + convert(Math.floor(n % 1000)) : '');
            if (n < 10000000) return convert(Math.floor(n / 100000)) + ' LAKH' + (Math.floor(n % 100000) !== 0 ? ' ' + convert(Math.floor(n % 100000)) : '');
            return convert(Math.floor(n / 10000000)) + ' CRORE' + (Math.floor(n % 10000000) !== 0 ? ' ' + convert(Math.floor(n % 10000000)) : '');
        };
