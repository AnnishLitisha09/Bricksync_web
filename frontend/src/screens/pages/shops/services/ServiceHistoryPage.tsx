...
export default function ServiceHistoryPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredLogs: Transaction[] = serviceData
        .filter((item: any) => item.serviceShopId === Number(shopId))
        .map((item: any) => ({ ...item, type: 'service', sortDate: new Date(item.date) }));

      const filteredStatements: Transaction[] = (statementData.data || statementData || [])
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-gray-50/50 p-4 md:p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <button onClick={() => navigate(-1)} className="group flex items-center gap-3 text-slate-400 hover:text-emerald-600 transition-all font-black uppercase tracking-[0.2em] text-[10px]">
          <div className="p-2 bg-white rounded-xl shadow-sm border border-slate-100 group-hover:bg-emerald-50 transition-all"><ArrowLeft size={16} /></div>
          Back to Hubs
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => navigate("/vehicles/services/add")} className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] hover:bg-emerald-600 transition-all shadow-xl shadow-slate-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-3 relative bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 flex flex-col md:flex-row md:items-center gap-6 overflow-hidden">
          ...
          <div className="relative z-10 p-5 bg-emerald-50 rounded-3xl text-emerald-600"><History size={40} strokeWidth={2.5} /></div>
          <div className="relative z-10 flex-1">
            ...
            <div className="flex items-center gap-4 mt-2">
              <p className="text-slate-400 font-bold text-xs">Service Value: <span className="text-slate-900">₹{summary.totalService.toLocaleString()}</span></p>
              <div className="w-1 h-1 bg-slate-300 rounded-full" />
              <p className="text-slate-400 font-bold text-xs">Total Paid: <span className="text-emerald-600">₹{summary.totalPaid.toLocaleString()}</span></p>
            </div>
          </div>
        </div>
        <div className="bg-emerald-600 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <p className="text-[10px] font-black text-emerald-100 uppercase tracking-[0.2em] opacity-80 mb-2">Total Outstanding</p>
            <p className="text-4xl font-black tabular-nums">₹{summary.outstanding.toLocaleString()}</p>
            <div className="mt-4 inline-flex items-center gap-2 px-3 py-1 bg-white/10 backdrop-blur-md rounded-lg text-[9px] font-black uppercase tracking-widest">Account Balance</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2 p-1.5 bg-white border border-slate-100 rounded-2xl w-fit shadow-sm overflow-x-auto">
        {(['all', 'logs', 'statements'] as TabType[]).map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-slate-900 text-white' : 'text-slate-400 hover:text-slate-600'}`}>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative bg-white w-full max-w-lg rounded-[3rem] shadow-2xl overflow-hidden">
              <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-emerald-50/30">
                <div>
                  <h2 className="text-2xl font-black text-slate-900 tracking-tight uppercase">Record Payment</h2>
                  <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mt-1">Vendor Settlement</p>
                </div>
              ...
              <form className="p-8 space-y-6" onSubmit={handlePaymentSubmit}>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Service Provider</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Notes</label>
                  <div className="relative">
                    <textarea
                      rows={2}
                      placeholder="Reference or purpose..."
                ...
                >
                  ...
                  {isSubmitting ? "Updating Ledger..." : "Confirm Settlement"}
                </button>
              </form>
            </motion.div>
          </div>
...
function ServiceLogRow({ log, idx }: { log: Transaction; idx: number }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.01 }} className="group bg-white p-2 pr-6 rounded-[2rem] shadow-sm border border-slate-100 hover:shadow-xl transition-all flex flex-col md:flex-row items-center gap-6">
      <div className="w-full md:w-32 h-24 rounded-[1.5rem] bg-slate-50 border border-slate-100 flex flex-col items-center justify-center group-hover:bg-emerald-50 group-hover:border-emerald-100 transition-colors">
      ...
      <div className="flex-1 flex flex-col lg:flex-row lg:items-center justify-between gap-4 py-2">
        <div>
          <h3 className="text-xl font-black text-slate-800 tracking-tight uppercase group-hover:text-emerald-600 transition-colors">{log.topic}</h3>
          <p className="text-slate-400 text-sm font-medium mt-1 line-clamp-1">{log.description || "Routine maintenance record."}</p>
        </div>
        <div className="flex items-center gap-6 p-4 bg-slate-50 rounded-2xl border border-slate-100 group-hover:bg-white transition-all">
          ...
          <div className="text-right">
            <p className="text-[8px] font-black text-slate-400 uppercase mb-1">Cost</p>
      ...
      <div className="w-full md:w-56 p-4 rounded-[1.5rem] bg-slate-900 text-white shadow-lg">
        ...
        <span className="text-[10px] font-mono text-emerald-400">{log.vehicle?.vehicleNumber}</span>
      </div>
    </motion.div>
  );
}
      ...
      <div className="flex-1 flex flex-col justify-center p-4">
        ...
        {st.description && (
          <div className="mt-3 px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl">
            <p className="text-[10px] font-bold text-slate-400 italic line-clamp-1">"{st.description}"</p>
          </div>
        )}
      </div>
      <div className="w-full md:w-56 p-4 rounded-[1.5rem] bg-emerald-900 text-white shadow-xl shadow-emerald-100">
        <p className="text-[9px] font-black text-white/40 uppercase tracking-widest mb-1">Authorized By</p>
        <p className="font-black text-[10px] uppercase truncate">{st.bank?.holderName || 'SYSTEM ACCOUNT'}</p>
      </div>
    </motion.div>
  );
}
