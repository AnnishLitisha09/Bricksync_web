...
export default function BunkPage() {
  ...
  const paginatedBunks = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    return filteredBunks.slice(startIndex, startIndex + itemsPerPage);
  }, [filteredBunks, currentPage]);

  const totalPages = Math.ceil(filteredBunks.length / itemsPerPage);
          ...
          {!loading && totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-8 border-t border-slate-100 mt-10">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center sm:text-left">
                Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredBunks.length)} of {filteredBunks.length} STATIONS
              </p>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
                  disabled={currentPage === 1}
                  className="p-3 bg-white rounded-2xl border border-slate-200 text-slate-400 disabled:opacity-30 hover:text-orange-600 transition-all shadow-sm"
                >
                  <ChevronLeft size={20} />
                </button>
